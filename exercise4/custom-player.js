...
let volSlider = document.getElementsByClassName("slider")[0];
...
function changevolume(amount) {
  var audioobject = document.getElementsByClassName("slider")[0];
...
// Get an Event Listener value to click  and then define it's functionality 
replay.addEventListener("click", replayMedia);
media.addEventListener("ended", replayMedia);

function replayMedia(){
  media.currentTime = 0;
  play.setAttribute('data-icon', 'r');
}

/*I Choose to add replay function because sometime we are listen to a video and we feels like we missed on
something very important, er can replay that. Or when we want to listen to it one more time, instead of using the timeline we can use a re play button. */